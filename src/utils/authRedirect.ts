...
function isLocalHost(host: string) {
  return host === 'localhost' || host === '127.0.0.1' || /^\d+\.\d+\.\d+\.\d+$/.test(host) || host.includes('localhost')
}
...
export function setSessionCookie(name: string, value: string, maxAgeSec = COOKIE_MAX_AGE_ONE_YEAR) {
  const host = location.hostname
  const isLocal = isLocalHost(host)
  
  if (isLocal) {
    // Set cookie for localhost development (1 year duration)
    // For localhost, we don't set domain to allow cross-port sharing
    const cookieString = `${name}=${encodeURIComponent(value)}; Path=/; Max-Age=${maxAgeSec}; SameSite=Lax`
    document.cookie = cookieString
    console.log('[auth][cookie][set][localhost]', { 
      host, 
      name, 
  ...
  // Force apex domain cookie for cross-subdomain sharing (1 year duration)
...
export function ensureCrossSubdomainCookies(names: string[]) {
  const host = location.hostname
  if (isLocalHost(host)) {
    console.log('[auth][cookie][promote] localhost detected - ensuring cookies are set', { host, names })
    // For localhost, just ensure the cookies exist and are properly set
    names.forEach(n => {
      const v = getCookie(n)
      if (v) {
        console.log('[auth][cookie][promote] found existing cookie', n)
        setSessionCookie(n, v) // will log itself
      } else {
        console.log('[auth][cookie][promote] no existing cookie found', n)
      }
    })
    console.log('[auth][cookie][promote] localhost done', cookieSummary(names))
    return
  }
  if (!isUnderApex(host)) {
    ...
    if (isLocalHost(location.hostname)) {
      if (candidate.startsWith('http')) {
        try {
          ...
          if (isLocalHost(u.hostname)) return candidate
          // For localhost, default to localhost with different port if needed
          return `http://localhost${candidate.startsWith('/') ? candidate : '/' + candidate}`
        } catch { 
          return `http://localhost${candidate.startsWith('/') ? candidate : '/' + candidate}`
        }
      }
